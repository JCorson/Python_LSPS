""" This file will import and arrange an ABF file experiment into a multi-dimensional
numpy array.
"""

from sys import stdout
from numpy import array, dstack, vstack #, size
from neo.io import AxonIO

def importABF(filename):
    print "Importing {}...".format(filename)
    stdout.flush()
    
    blk = readABF(filename)
    Raw_Data = readBlock(blk)
    return Raw_Data

def readABF(filename):
    """ Imports the *.abf file and returns a neo Block format
    """
    reader = AxonIO(filename)
    return reader.read_block(lazy=False, cascade = True)

def readBlock(blk):
    Raw_Data = {} # Create a blank dictionary
    ...
    # Create Raw_Data.keys from Channel_Names
    for name in Channel_Names:
        Raw_Data[name] = {}
        
    # Get variable names and set up initial dictionary key for each analog signal
    for i in range(len(Sweep1)):
        Raw_Data[Sweep1[i].name]['Units'] = Sweep1[i].units
        Raw_Data[Sweep1[i].name]['Times'] = array(range(len(Sweep1[i].times)))/(array(Sweep1[i].sampling_rate)/1000.) # Time array in ms
        Raw_Data[Sweep1[i].name]['Sampling Rate'] = Sweep1[i].sampling_rate # Sampling Rate in Hz
        Raw_Data[Sweep1[i].name]['Signal'] = array(Sweep1[i])
    ...
    if len(blk.segments) > 1:   
        for sweep in blk.segments[1:]:
            for channel in sweep.analogsignals:
                Raw_Data[channel.name]['Signal'] = vstack((Raw_Data[channel.name]['Signal'], array(channel)))   
        for channel in sweep.analogsignals:
            Raw_Data[channel.name]['Signal'] = Raw_Data[channel.name]['Signal'].T  
    return Raw_Data
...
def addRun(filename, Raw_Data):   
    ...
    for i in range(len(Sweep1)):
        Run[Sweep1[i].name]['Signal'] = array(Sweep1[i])
    
    # Add the remaining sweeps to the Signal Key for each analog signal. Sweeps are added in n.dim = 1  
    for sweep in blk.segments[1:]:
        for channel in sweep.analogsignals:
            Run[channel.name]['Signal'] = vstack((Run[channel.name]['Signal'], array(channel)))
    for channel in sweep.analogsignals:
        Run[channel.name]['Signal'] = Run[channel.name]['Signal'].T  
    
    # Combine this Run with the previous Runs in Raw_Data     
    for name in Channel_Names:
        Raw_Data[name]['Signal'] = dstack((Raw_Data[name]['Signal'], Run[name]['Signal']))
    
    return Raw_Data

#################### Code for LSPS, Moved to Separate File #####################    
...
#                Parameters['Random_Stimulations'][(key)] = dstack((Parameters['Random_Stimulations'][key], tempParam['Random_Stimulations'][key]))
#            tempImage = loadMatlab_LSPS_Image(File)
#            Images = dstack((Images, tempImage))
...
#        for run in range(size(Recording[(Input)]['Signal'][0,0,:])):
...
#        for run in range(size(Recording[(channel)]['Signal'][0,0,:])):
...
#            Recording[(channel)]['Signal'] = dstack((Recording[channel]['Signal'], tempRecording[channel]['Signal']))
#        
